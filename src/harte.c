...
void failureLog(FAIL_STATE *state_accumulator, FAIL_STATE failedState, MACHINE *m, STATE *state) {
    ...
    if (failedState & Y) {
        logToStr(CHAN_FAIL, 0, "aiy:%02X EY:%02X ", m->cpu.Y, state->Y);
    }
    if (failedState & X) {
...
extern MACHINE m;
extern double expected_address, actual_address;
extern double expected_value,   actual_value;
extern const char *expected_action;
...
int harte_check(MACHINE *m) {
    ...
    cJSON *cycle = cJSON_GetArrayItem(g_cycles, g_cycle_index);
    if (!cJSON_IsArray(cycle) || cJSON_GetArraySize(cycle) != 3) {
        return RESULT_PREP_ERROR;
    }

    double exp_addr = cJSON_GetNumberValue(cJSON_GetArrayItem(cycle, 0));
    double exp_val  = cJSON_GetNumberValue(cJSON_GetArrayItem(cycle, 1));
    const char *exp_action_str = cJSON_GetStringValue(cJSON_GetArrayItem(cycle, 2));
    if (isnan(exp_addr) || isnan(exp_val) || exp_action_str == NULL) {
        return RESULT_PREP_ERROR;
    }
...
int runTest(cJSON *aTest, int testNo, FAIL_STATE *failState) {
    ...
    logToStr(CHAN_TEST_INFO, 0, "Test %5d: %s ", testNo, name);
    ...
    if (r != RESULT_SUCCESS) {
        logToStr(CHAN_TEST_INFO, 0,
                 ...
                 g_mismatch_index, g_cycle_count,
                 expected_address, expected_value, expected_action ? expected_action[0] : '?',
                 actual_address,   actual_value,   actual_action);
        return RESULT_FAIL;
    }
...
// Process a single json test file, one test at a time
    ...
    if (file != NULL) {
        ...
        cJSON_ArrayForEach(aTest, arrayContainer) {
            ...
            if(result == RESULT_FILTERED_OUT) {
                testsSkipped += cJSON_GetArraySize(arrayContainer);
                return RESULT_SUCCESS;
            }
        }
...
int main(int argc, char **argv) {
...
#if defined(SHORT_FORM) || !defined(SHOW_INDIVIDUAL_TESTS)
    ...
    logMessage(CHAN_TEST_INFO, "TEST RESULTS:\n"\
               ...
               "Skipped: %d\n"\
               "Percent: %3.2f%%\n",
               testNumber, testsPassed, testsFailed, testsSkipped, (double)((testsPassed + testsSkipped) * 100) / (testsPassed + testsFailed + testsSkipped));
#endif
